...
*/
  const userProfileInfoGet = async ({ queryKey: { 1: key} }) => {
    const allContacts = await axios.post(`${ipAddress}/all_contact`,{"org": key.org,"username": key.username});
    return(allContacts)
  }
  ...
  const updateContact = async (contact) => {
    ...
    try {
      await axios.post(`${ipAddress}/update_api`,contact,{headers : {Authorization : `token ${token}`}})  
    } catch (error) {
  ...
  const deleteContact = async(id) => {
    try {
      await axios.post(`${ipAddress}/admin_delete_user`,{"org":org, "user":id},{headers : {"Authorization" : `token ${token}`}})  
    } catch (error) {
      console.error("Error updating contact:", error);
  }
